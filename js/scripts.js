...
*/
  ...
  (function () {
    ...
    //	#Waking_Segment {opacity: 0; visibility: hidden;}
  	...
  	//	Variables
  		...
  		const chickpea_prog_path = anim_el.querySelector("#Chickpea_Progress_Path");
  		const sushi_prog_path = anim_el.querySelector("#Sushi_Progress_Path");

  		const walking_stats_popup = anim_el.querySelector("#Walking_Stats_Popup");
  ...
  //	GSAP Sets
		...
		gsap.set(walking_stats_popup, { autoAlpha:0, yPercent: 20 });
		...
		gsap.set(chickpea_prog_path, { drawSVG: "0% 0%" });
		gsap.set(sushi_prog_path, { drawSVG: "0% 0%" });
	...
	//	First Progress Point Timeline
  	...
  	first_prog_point_tl.to(first_pear_graph_symbol, {
		...
		first_prog_point_tl.to(chickpea_prog_path, {
				drawSVG: "0% 100%",
				duration: 2,
				ease: "power3.inOut"
		}, "<");
	...
	//	Second Progress Point Timeline
  	...
  	second_prog_point_tl.to(second_pear_graph_symbol, {
		...
		second_prog_point_tl.to(sushi_prog_path, {
				drawSVG: "0% 100%",
				duration: 2,
				ease: "power3.inOut"
		}, "<");
  ...
  //	Fourth Progress Point Timeline
	...
	//	Sixth Progress Point Timeline
		...
		sixth_prog_point_tl.to(walking_stats_popup, {
				autoAlpha: 1,
				yPercent: 0,
				force3D: true,
				duration: 0.8,
	...
	//	Seventh Progress Point Timeline
		let seventh_prog_point_tl = gsap.timeline({ paused: true});

		seventh_prog_point_tl.to(safe_graph_symbol, {
			autoAlpha: 1,
			scale: 1,
			force3D: true,
			duration: 0.3,
  	...
  	seventh_prog_point_tl.to(insight_walking_lowered_popup, {
			autoAlpha: 1,
			yPercent: 0,
			force3D: true,
			duration: 0.8,
	...
	//	Re Set Elements on main timeline repeat
		function main_tl_onRepeat_func() {
			...
			seventh_prog_point_tl.restart();
			seventh_prog_point_tl.pause();

		}


	//	Progress Points and Triggered Variable
		let progress_points =[0.055, 0.415, 0.675, 0.730, 0.785, 0.890, 0.923];
  ...
  //	Create Graph Timeline
		...
		glucose_tl.to(glucose_lines_mask_el, {
		       ...
		       // Point 6
		        ...
		        // Point 7
			        if (progress >= progress_points[6] && !triggered.has(progress_points[6])) {

		        		glucose_tl.pause();
	          		gsap.delayedCall(pause_duration, () => glucose_tl.play());
		            
		            seventh_prog_point_tl.play();

		            triggered.add(progress_points[6]);
